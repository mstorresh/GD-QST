...
def gd_manifold_adaptive(data, ops_jnp, params: jnp.ndarray, iterations: int, 
  ...
  @jit
  def step(params, step_size, data, ops_jnp):
    ...
    params = stiefel_update(params, grads, step_size)
    # print(initiald)
    # initiald = alpha*initiald
    return params
  
  # adaptive learning starts
  learning_rate = lr
  ...
  for i in tqdm(range(iterations), disable=tqdm_off):
    ...
    ops2 = ops_jnp[indix]
    params = step(params, learning_rate, data_b, ops2)

    #Compute the current loss
    current_loss = cost(params, data_b, ops2, lamb)

    # Update learning rate adaptively
    if current_loss >= prev_loss:
        learning_rate *= decay  # Reduce the learning rate by a factor
        # if learning_rate < 1e-4:  # Lower bound for the learning rate
        #     break  # Stop optimization if the learning rate is too small
    else:
        learning_rate = min(boost * lr, 0.5)  # Increase the learning rate up to 0.5
    ...
    if not tqdm_off:
        pbar_GD.set_description("Fidelity GD-manifold-adaptive {:.4f}".format(f))
        pbar_GD.update()
